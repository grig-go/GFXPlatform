-- =============================================
-- Update create_invitation to replace existing pending invitations
-- Instead of throwing an error, delete the old invitation and create a new one
-- =============================================

DROP FUNCTION IF EXISTS create_invitation(UUID, TEXT, UUID, TEXT);
...
BEGIN
  ...
  -- This allows "resending" by creating a new invitation
  DELETE FROM u_invitations
  WHERE email = lower(trim(p_email))
    AND organization_id = p_organization_id
...
GRANT EXECUTE ON FUNCTION create_invitation(UUID, TEXT, UUID, TEXT) TO service_role;

COMMENT ON FUNCTION create_invitation(UUID, TEXT, UUID, TEXT) IS 'Creates an invitation, replacing any existing pending invitation for the same email/org';
