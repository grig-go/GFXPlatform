...
import {
  ...
  PenLine, FilePlus, Save, Play, Pin,
} from 'lucide-react';
import {
  Tooltip,
  TooltipContent,
...
import type { Template, Element } from '@/types';
...
export function OutlinePanel() {
  ...
  const { project, layers, addLayer, addTemplate, updateProjectSettings, isDirty, saveProject, isSaving } = useDesignerStore();
  ...
  return (
    <div className="h-full w-full min-w-0 min-h-0 flex flex-col bg-card border-l border-border overflow-hidden">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        <TabsList className="mx-2 mt-1.5 h-6">
          <TabsTrigger value="layers" className="flex-1 gap-1 text-[10px]">
            <Layers className="w-3 h-3" />
            Layers
          </TabsTrigger>
          <TabsTrigger value="elements" className="flex-1 gap-1 text-[10px]">
            <LayoutTemplate className="w-3 h-3" />
            Elements
          </TabsTrigger>
          <TabsTrigger value="preview" className="flex-1 gap-1 text-[10px]">
      ...
      {/* Properties Section */}
      <div className="border-t border-border flex-shrink-0 max-h-[50%] overflow-hidden flex flex-col">
  ...
  return (
    <div
      className={cn(
        ...
        isOnAir && !isSelected && 'bg-emerald-500/10',
        !isSelected && !isOnAir && 'hover:bg-muted/50',
    ...
    >
      <LayoutTemplate className={cn(
        "w-3 h-3",
        isOnAir ? "text-emerald-400" : "text-muted-foreground"
      )} />
