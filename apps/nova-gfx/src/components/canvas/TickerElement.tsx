...
function FlipTicker({ items, config, className, style, onItemClick }: TickerModeProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Calculate next index before starting animation
  ...
  // For bottom-up: current exits by moving down (translate-y positive), next enters from below (starts translated up)
  const flipUp = config.direction !== 'down'; // Default to 'up' (bottom-up flip)
  ...
  return (
    <div className={cn('relative overflow-hidden', className)} style={style}>
      ...
      <div
        ...
        style={{
          transform: isFlipping
            ? (flipUp ? 'translateY(-100%)' : 'translateY(100%)')
            : 'translateY(0)',
      ...
      {/* Next item - enters from bottom (for up direction) */}
      <div
