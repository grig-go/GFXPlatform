...
import type { Animation, Keyframe, Element } from '@/types';
...
export function getAnimatedProperties(
  ...
  if (phaseAnimations.length === 0) {
    if (debug) console.log('[Animation] No animations for element', element.id, 'in phase', phase);
    return {};
  }
  ...
  phaseAnimations.forEach((anim) => {
    ...
    if (animKeyframes.length === 1) {
      ...
      if (debug) console.log('[Animation] Only 1 keyframe, applying:', result);
      return;
    }
    ...
    let progress = localTime / anim.duration;
