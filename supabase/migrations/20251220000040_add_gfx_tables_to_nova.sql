-- =====================================================
-- Migration: Add Nova-GFX tables to Nova database
-- These tables are for the Pulsar GFX broadcast graphics system
-- They use u_organizations and u_users (Nova's user system)
-- =====================================================

-- ============================================
-- PROJECTS (gfx_projects)
-- ============================================
CREATE TABLE IF NOT EXISTS gfx_projects (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID REFERENCES u_organizations(id) ON DELETE CASCADE,
  created_by UUID REFERENCES u_users(id) ON DELETE SET NULL,
  name TEXT NOT NULL,
  description TEXT,
  slug TEXT NOT NULL,
  ...
  thumbnail_url TEXT,
  updated_by UUID REFERENCES u_users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS gfx_elements (
  ...
  element_type TEXT CHECK (element_type IN ('div', 'text', 'image', 'shape', 'group', 'video', 'lottie', 'd3-chart', 'ticker')) NOT NULL,
...
-- SUPPORT TICKETS
-- ============================================
CREATE TABLE IF NOT EXISTS gfx_support_tickets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES u_users(id) ON DELETE SET NULL,
  project_id UUID REFERENCES gfx_projects(id) ON DELETE SET NULL,
  title TEXT NOT NULL,
  description TEXT NOT NULL,
  ticket_type TEXT CHECK (ticket_type IN ('bug', 'feature', 'question', 'other')) DEFAULT 'other',
  priority TEXT CHECK (priority IN ('low', 'medium', 'high', 'critical')) DEFAULT 'medium',
  status TEXT CHECK (status IN ('open', 'in_progress', 'resolved')) DEFAULT 'open',
  app TEXT,
  resolved_at TIMESTAMPTZ,
  resolved_by UUID REFERENCES u_users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS organization_textures (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID REFERENCES u_organizations(id) ON DELETE CASCADE NOT NULL,
  name TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_org_textures_org_id ON organization_textures(organization_id);
CREATE INDEX IF NOT EXISTS idx_org_textures_media_type ON organization_textures(media_type);
CREATE INDEX IF NOT EXISTS idx_org_textures_created_at ON organization_textures(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_org_textures_name ON organization_textures(name);
CREATE INDEX IF NOT EXISTS idx_org_textures_tags ON organization_textures USING GIN(tags);
...
ALTER PUBLICATION supabase_realtime ADD TABLE gfx_chat_messages;
...
CREATE POLICY "gfx_projects_auth_all" ON gfx_projects FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_project_design_systems_auth_all" ON gfx_project_design_systems FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_layers_auth_all" ON gfx_layers FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_folders_auth_all" ON gfx_folders FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_templates_auth_all" ON gfx_templates FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_elements_auth_all" ON gfx_elements FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_animations_auth_all" ON gfx_animations FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_keyframes_auth_all" ON gfx_keyframes FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_bindings_auth_all" ON gfx_bindings FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_chat_history_auth_all" ON gfx_chat_history FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_chat_messages_auth_all" ON gfx_chat_messages FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_playback_state_auth_all" ON gfx_playback_state FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_playback_commands_auth_all" ON gfx_playback_commands FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_template_versions_auth_all" ON gfx_template_versions FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_animation_presets_auth_all" ON gfx_animation_presets FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "gfx_support_tickets_auth_all" ON gfx_support_tickets FOR ALL TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY "org_textures_auth_all" ON organization_textures FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Anonymous access for playback (Nova Player needs this)
CREATE POLICY "gfx_projects_anon_select" ON gfx_projects FOR SELECT TO anon USING (true);
CREATE POLICY "gfx_layers_anon_select" ON gfx_layers FOR SELECT TO anon USING (true);
CREATE POLICY "gfx_templates_anon_select" ON gfx_templates FOR SELECT TO anon USING (true);
CREATE POLICY "gfx_elements_anon_select" ON gfx_elements FOR SELECT TO anon USING (true);
CREATE POLICY "gfx_animations_anon_select" ON gfx_animations FOR SELECT TO anon USING (true);
CREATE POLICY "gfx_keyframes_anon_select" ON gfx_keyframes FOR SELECT TO anon USING (true);
CREATE POLICY "gfx_bindings_anon_select" ON gfx_bindings FOR SELECT TO anon USING (true);
CREATE POLICY "gfx_playback_state_anon_all" ON gfx_playback_state FOR ALL TO anon USING (true) WITH CHECK (true);
CREATE POLICY "gfx_playback_commands_anon_all" ON gfx_playback_commands FOR ALL TO anon USING (true) WITH CHECK (true);
CREATE POLICY "gfx_project_design_systems_anon_select" ON gfx_project_design_systems FOR SELECT TO anon USING (true);
CREATE POLICY "gfx_animation_presets_anon_select" ON gfx_animation_presets FOR SELECT TO anon USING (true);
