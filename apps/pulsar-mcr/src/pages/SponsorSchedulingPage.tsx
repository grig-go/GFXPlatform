...
import { useSponsorSchedules } from '../hooks/useSponsorSchedules';
import {
  SponsorSchedule,
...
import { SponsorScheduleDialog } from '../components/SponsorScheduleDialog';
...
const MediaCellRenderer: React.FC<ICellRendererParams> = (params) => {
  const schedule = params.data as SponsorSchedule;
...
// Helper to check if a schedule is a fallback (priority 0 with no restrictions)
const isFallbackSchedule = (schedule: SponsorSchedule): boolean => {
  if (schedule.priority !== 0) return false;

  // Check if no date restrictions
  const hasDateRestriction = schedule.start_date || schedule.end_date;

  // Check if no day restrictions (all false = every day = no restriction)
  const hasDayRestriction = Object.values(schedule.days_of_week).some(v => v);

  // Check if no time restrictions
  const hasTimeRestriction = schedule.time_ranges.some(r => r.start && r.end);

  return !hasDateRestriction && !hasDayRestriction && !hasTimeRestriction;
};
...
const StatusCellRenderer: React.FC<ICellRendererParams> = (params) => {
  ...
  const isFallback = schedule && isFallbackSchedule(schedule);
...
const SponsorSchedulingPage: React.FC = () => {
  ...
  const [editingSchedule, setEditingSchedule] = useState<SponsorSchedule | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [scheduleToDelete, setScheduleToDelete] = useState<SponsorSchedule | null>(null);
  const [selectedRows, setSelectedRows] = useState<SponsorSchedule[]>([]);
  ...
  const columnDefs: ColDef[] = useMemo(() => [
          ...
          <AgCheckbox
            ...
            onChange={async (newValue) => {
              try {
                ...
                console.error('Failed to update sponsor schedule active status:', error);
              }
            }}
          />
        );
    ...
    },
    {
      ...
      filterParams: {
        values: SPONSOR_CATEGORIES.map(c => c.value),
        valueFormatter: (params: any) => {
          const cat = SPONSOR_CATEGORIES.find(c => c.value === params.value);
          return cat?.label || 'General';
        }
      },
      cellRenderer: (params: ICellRendererParams) => {
        const schedule = params.data as SponsorSchedule;
        const categoryInfo = SPONSOR_CATEGORIES.find(c => c.value === (schedule?.category || ''));
        const label = categoryInfo?.label || 'General';
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center', height: '100%' }}>
            <Chip
              label={label}
              size="small"
              variant={schedule?.category ? 'filled' : 'outlined'}
              color={schedule?.category ? 'info' : 'default'}
            />
          </Box>
        );
      }
  ...
  const getContextMenuItems = useCallback((params: any): (MenuItemDef | DefaultMenuItem)[] => {
    const schedule = params.node?.data as SponsorSchedule;
    if (!schedule) return [];
  ...
  const handleEdit = (schedule: SponsorSchedule) => {
    setEditingSchedule(schedule);
    setDialogMode('edit');
    setDialogOpen(true);
  };

  const handleDeleteClick = (schedule: SponsorSchedule) => {
  ...
  const handleSave = async (data: SponsorScheduleFormData) => {
  ...
  const handleValidate = (data: SponsorScheduleFormData) => {
    ...
    (<div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div className="toolbar">
        <div className="toolbar-left">
          <Tooltip title="Add New Sponsor Schedule">
            <span>
              <IconButton
        ...
        <div className="ag-theme-alpine" style={{ height: '100%', width: '100%', position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>
          <AgGridReact
            ...
            overlayNoRowsTemplate={
              selectedChannel === 'all'
                ? '<span>No sponsor schedules found. Click + to create one.</span>'
...
export default SponsorSchedulingPage;
