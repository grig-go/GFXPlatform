...
interface BannerScheduleDialogProps {
  open: boolean;
  onClose: () => void;
  onSave: (data: BannerScheduleFormData) => Promise<void>;
  initialData?: BannerSchedule | null;
  channels: Channel[];
  mode: 'create' | 'edit';
  conflicts?: BannerScheduleConflict[];
  onValidate?: (data: BannerScheduleFormData) => BannerScheduleConflict[];
}

export const BannerScheduleDialog: React.FC<BannerScheduleDialogProps> = ({
  open,
  onClose,
  onSave,
  initialData,
  ...
  const [formData, setFormData] = useState<BannerScheduleFormData>({
    channel_ids: [],
    media_id: '',
    name: '',
    start_date: '',
  ...
  const [conflicts, setConflicts] = useState<BannerScheduleConflict[]>([]);
  ...
  }, [open, initialData, channels, externalConflicts]);
  ...
  const handleFieldChange = (field: keyof BannerScheduleFormData, value: any) => {
  ...
  const handleTriggerChange = (index: number, field: 'start' | 'end', value: string) => {
    setFormData(prev => ({
      ...prev,
  ...
  const handleAddTrigger = () => {
    setFormData(prev => ({
      ...prev,
      triggers: [...prev.triggers, { start: '', end: '' }]
    }));
  };
        ...
        <DialogTitle>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h6">
              {mode === 'create' ? 'Create Banner Schedule' : 'Edit Banner Schedule'}
            </Typography>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                placeholder="e.g., Morning Banner, Holiday Special"
              />
            </Grid>
            ...
            {/* Triggers - When within each hour to show the banner */}
            <Grid item xs={12}>
              <Divider sx={{ my: 1 }} />
              <Box display="flex" justifyContent="space-between" alignItems="center" sx={{ mt: 2, mb: 2 }}>
                <Typography variant="subtitle2">Hourly Triggers</Typography>
                <Typography variant="body2" color="text.secondary">
                  When within each hour to display (MM:SS)
                </Typography>
              </Box>
            </Grid>
            ...
            <Grid item xs={12}>
              {formData.triggers.length === 0 ? (
                <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 1, mb: 2 }}>
                  <Typography variant="body2" color="text.secondary" align="center">
                    No triggers defined - banner will never be displayed
                  </Typography>
                </Box>
              ) : (
                  ...
                  <Box key={index} sx={{ mb: 2 }}>
                    <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                      <TextField
                        ...
                        placeholder="00:00"
                        value={trigger.start}
                        onChange={(e) => handleTriggerChange(index, 'start', e.target.value)}
                        InputLabelProps={{ shrink: true }}
                        sx={{ minWidth: 120 }}
                        inputProps={{
                        ...
                        helperText="e.g., 00:00"
                      />
                      <TextField
                        label="End (MM:SS)"
                        placeholder="05:00"
                        value={trigger.end}
                        onChange={(e) => handleTriggerChange(index, 'end', e.target.value)}
                        InputLabelProps={{ shrink: true }}
                        sx={{ minWidth: 120 }}
                        inputProps={{
                          pattern: '[0-5][0-9]:[0-5][0-9]',
              ...
              {formData.triggers.length > 0 && (
                <Alert severity="info" icon={<InfoIcon />} sx={{ mt: 2 }}>
                  <Typography variant="body2">
                    <strong>Triggers</strong> define when within each hour the banner will be displayed.
                    For example, "00:00 - 05:00" means the banner shows for the first 5 minutes of every hour.
                  </Typography>
                </Alert>
              )}
            </Grid>
...
export default BannerScheduleDialog;
