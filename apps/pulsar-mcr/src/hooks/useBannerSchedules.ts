...
export const useBannerSchedules = (): UseBannerSchedulesReturn => {
  ...
  const fetchSchedules = useCallback(async (filters?: BannerScheduleFilters) => {
    try {
      ...
      console.log('[useBannerSchedules] Fetching with effectiveOrgId:', effectiveOrgId);
      ...
      console.log('[useBannerSchedules] Fetched schedules:', schedulesWithMedia?.length || 0, 'for org:', effectiveOrganization?.id);
