/**
 * Binding Resolution Utilities for Pulsar GFX
...
export interface Binding {
  ...
  binding_type: 'text' | 'image' | 'number' | 'color' | 'boolean';
...
/**
 * Resolve all bindings for a payload object
 * Takes a template's bindings and current data record, returns updated payload
 */
export function resolvePayloadBindings(
  ...
  if (!currentRecord || bindings.length === 0) return payload;
  ...
  for (const binding of bindings) {
    ...
    if (rawValue !== undefined) {
      const formattedValue = applyFormatter(rawValue, binding.formatter, binding.formatter_options);
      resolvedPayload[binding.element_id] = String(formattedValue);
    }
  }
