...
import type { DesignTool, HistoryEntry, DesignerSnapshot } from '@/types/designer';
import {
  loadChatHistory,
  saveChatMessage,
  clearChatHistory,
      ...
      immer((set, get) => ({
        ...
        loadProject: async (projectId) => {
          ...
          try {
            ...
            // ALWAYS check localStorage first (for starter projects and local saves)
        ...
        saveProject: async () => {
          ...
          set({ isSaving: true });
          const thumbnail = await captureCanvasSnapshot(0.7, 480);
          
          // Update project with thumbnail and timestamp
          const updatedProject = {
            ...state.project,
            thumbnail_url: thumbnail || state.project.thumbnail_url,
            updated_at: new Date().toISOString(),
          };
        ...
        deleteTemplate: (templateId) => {
          set((state) => {
            ...
            const elementIds = state.elements
              ...
              .map((e) => e.id);
            state.elements = state.elements.filter((e) => e.template_id !== templateId);
            
            // Delete animations for those elements
            const animationIds = state.animations
              .filter((a) => elementIds.includes(a.element_id))
              .map((a) => a.id);
            state.animations = state.animations.filter((a) => !elementIds.includes(a.element_id));
            
            // Delete keyframes for those animations
            state.keyframes = state.keyframes.filter((k) => !animationIds.includes(k.animation_id));
        ...
        selectElements: (ids, mode = 'replace') => {
          set((state) => {
            if (mode === 'replace') {
              ...
              state.selectedElementIds = Array.from(current);
            }
          });
        },
        ...
        fitToScreen: () => {
          const viewportWidth = window.innerWidth - 580;
          ...
          set({
            zoom,
            panX: (viewportWidth - project.canvas_width * zoom) / 2,
            panY: (viewportHeight - project.canvas_height * zoom) / 2,
          });
        },
